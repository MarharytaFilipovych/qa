...
class RabbitMQConfigTest {
    ...
    @Test
    void orderCreatedBinding_bindsCorrectly() {
        Queue queue = config.orderCreatedQueue();
        TopicExchange exchange = config.coreExchange();
    ...
    @Test
    void messageConverter_mapsToOrderCreatedEvent() {
        // The custom ClassMapper always maps to OrderCreatedEvent
        Jackson2JsonMessageConverter converter = config.messageConverter();
        org.springframework.amqp.core.MessageProperties props =
                new org.springframework.amqp.core.MessageProperties();
        Class<?> mapped = converter.getClassMapper().toClass(props);
        assertThat(mapped).isEqualTo(
                com.microservices.margo.notification_service.core.application.event.OrderCreatedEvent.class);
    }
}